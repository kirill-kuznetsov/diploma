...
int bucket_rushr_choose(struct crush_bucket_rushr *bucket,
        int x, int replica_num, int* result) {
    int item = bucket->h.size - 1;//TODO: check
    int replica = 0;
    int replicas_left = replica_num;
    while(replicas_left != 0)
    {
        int item_id = bucket->h.items[item];
        replica = replica_num - replicas_left;
        if (replicas_left < item + 1) {
            __u64 z = crush_hash32_4(bucket->h.hash, x, item_id,
                    replica, bucket->h.id);
            z &= 0xffff;
            z *= bucket->sum_weights[item];
            z = z >> 16;

            if (z < bucket->item_weights[item]) {
                result[replica] = item_id;
                replicas_left--;
            }
        } else {
            result[replica] = item_id;
...
/*
        case CRUSH_BUCKET_RUSHR:
            return bucket_rushr_choose((struct crush_bucket_rushr *) in,
                    x, r);
*/
        default:
            BUG_ON(1);
...
static int crush_choose(struct crush_map *map,
    ...
    if (bucket->type == CRUSH_BUCKET_RUSHR) {
/*
